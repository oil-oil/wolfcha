...
export function WolfPlanningPanel({ gameState, humanPlayer }: WolfPlanningPanelProps) {
  ...
  return (
    <div className="bg-[#1a1512] border border-[#3e2723] rounded-lg p-4 text-[#f0e6d2]">
      {/* 标题 */}
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          <div className="text-sm font-bold">狼人协作</div>
          <div className="text-xs text-[#a09080]">选择今晚的击杀目标</div>
        </div>
      </div>

      {/* 狼队友状态 */}
      <div className="space-y-2 mb-4">
        <div className="text-xs text-[#a09080] mb-2">队友投票状态:</div>
        {wolves.map(wolf => {
          ...
          return (
            ...
            >
              <div className="w-6 h-6 bg-[var(--color-wolf)] rounded-full flex items-center justify-center text-[10px] font-bold text-white">
                {wolf.seat + 1}
              </div>
              <span className="text-sm flex-1">
                {wolf.displayName}
                {wolf.isHuman && <span className="ml-1 text-[10px] text-[var(--color-accent)]">(你)</span>}
              </span>
              {hasVoted ? (
                <span className="flex items-center gap-1 text-xs text-green-400">
                  <CheckCircle size={12} weight="fill" />
                  <Target size={10} />
                  <span className="font-bold">{wolfVotes[wolf.playerId] + 1}号</span>
                  {votedTarget && <span className="text-[10px] text-[#a09080]">{votedTarget.displayName}</span>}
                </span>
              ) : (
      ...
      {sortedTargets.length > 0 && (
        <div className="border-t border-[#3e2723] pt-3">
          <div className="text-xs text-[#a09080] mb-2">目标汇总:</div>
          <AnimatePresence mode="popLayout">
            {sortedTargets.map(([targetSeat, { voters, target }]) => (
              ...
              >
                ...
                <span className="font-bold">{Number(targetSeat) + 1}号</span>
